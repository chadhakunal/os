...
static int copy_recursive(const char *src, const char *dst) {
  ...
  if (mkdir(dst, 0755) < 0) {
    ...
    if (fd < 0) { printf("mv: cannot create directory '%s'\n", dst); return 1; }
    close(fd);
  }

  int dir_fd = open(src, O_RDONLY);
  if (dir_fd < 0) { printf("mv: cannot open '%s'\n", src); return 1; }
...
static int remove_file(const char *path) {
  if (unlink(path) < 0) { printf("mv: cannot remove '%s'\n", path); return 1; }
  return 0;
}
...
static int remove_dir(const char *path) {
  ...
  if (rmdir(path) < 0) { printf("mv: cannot remove dir '%s'\n", path); return 1; }
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  if (copy_recursive(src, dst) != 0) {
    printf("mv: failed to copy '%s' to '%s'\n", src, dst);
    return 1;
  }
