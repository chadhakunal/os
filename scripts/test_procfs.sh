...
PID=$(run_cmd "cat /proc/self/status" | grep '^Pid:' | awk '{print $2}')
OUT=$(run_cmd "ls /proc")
...
result "uptime: non-empty"                  "$([ -n "$OUT" ] && echo 1 || echo 0)"
result "uptime: matches SECS.CS IDLE.CS"    "$(matches "$OUT" '^[0-9]+\.[0-9]+ [0-9]+\.[0-9]+')"
result "uptime: has two fields"             "$([ "$(echo "$OUT" | awk '{print NF}')" = "2" ] && echo 1 || echo 0)"

# centiseconds must be 0-99
UP_CS=$(echo "$OUT" | sed 's/[0-9]*\.\([0-9]*\) .*/\1/')
result "uptime: centiseconds <= 99"         "$([ "$UP_CS" -le 99 ] 2>/dev/null && echo 1 || echo 0)"

# two reads: second >= first (integer seconds)
UP1=$(run_cmd "cat /proc/uptime" | awk -F. '{print $1}')
UP2=$(run_cmd "cat /proc/uptime" | awk -F. '{print $1}')
...
TOTAL=$(echo "$OUT" | grep '^MemTotal:' | awk '{print $2}')
FREE=$(echo  "$OUT" | grep '^MemFree:'  | awk '{print $2}')
