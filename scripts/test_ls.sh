...
# Setup
send "rm -rf /mnt/tmp/lstest && mkdir -p /mnt/tmp/lstest/subdir && echo hello > /mnt/tmp/lstest/file1.txt && echo world > /mnt/tmp/lstest/file2.txt"
wait_for_prompt

# 1. ls lists expected files
OUT=$(run_cmd "ls /mnt/tmp/lstest")
...
send "mkdir -p /mnt/tmp/lstest/emptydir"
wait_for_prompt
OUT=$(run_cmd_raw "ls -a /mnt/tmp/lstest/emptydir")
...
OUT=$(run_cmd_exitcode "ls /mnt/tmp/lstest/nosuchdir ; echo exitcode=\$?")
result "ls missing dir exits nonzero" "$(contains "$OUT" "exitcode=1")"

# 4. ls -l shows permission bits
OUT=$(run_cmd "ls -l /mnt/tmp/lstest")
...
OUT=$(run_cmd "ls /mnt/tmp/lstest/file1.txt")
result "ls single file prints name" "$(contains "$OUT" "file1.txt")"

# 7. ls multiple paths
OUT=$(run_cmd "ls /mnt/tmp/lstest/file1.txt /mnt/tmp/lstest/file2.txt")
...
OUT=$(run_cmd_raw "ls /mnt/tmp/lstest")
BLANK=$(echo "$OUT" | grep -c '^[[:space:]]*$' || true)
result "ls no blank lines in output" "$([ "$BLANK" = "0" ] && echo 1 || echo 0)"

# Cleanup
send "rm -rf /mnt/tmp/lstest"
wait_for_prompt
