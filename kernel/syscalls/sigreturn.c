...
int64_t sys_rt_sigreturn(struct trap_frame *tf) {
  debugk("syscall: rt_sigreturn() - restoring context for PID %llu, depth=%u\n",
  ...
  copy_from_user(&old_blocked_mask, (void *)(frame_addr + 296), sizeof(uint64_t));
  current_task->signal_state.blocked = old_blocked_mask;

  current_task->signal_handler_depth--;

  debugk("syscall: rt_sigreturn() - restored context, sp=%llx, sepc=%llx, new_depth=%u\n",
         current_task->tf.sp, current_task->tf.sepc, current_task->signal_handler_depth);
